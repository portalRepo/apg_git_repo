...
def all_views_max_pool(input_layer, network_type, stride=(2, 2)):
    ...
    if network_type == "CC":
        ...
        output_l_mlo = tf.nn.max_pool(input_l_mlo, ksize=[1, stride[0], stride[1], 1],
                                     strides=[1, stride[0], stride[1], 1],
                                     padding='SAME')
        output_r_mlo = tf.nn.max_pool(input_r_mlo, ksize=[1, stride[0], stride[1], 1],
                                     strides=[1, stride[0], stride[1], 1],
                                     padding='SAME')
...
def all_views_global_avg_pool(input_layer, network_type):
...
def all_views_flattening_layer(input_layer, network_type):
    ...
    if network_type == "CC":
        ...
        h_cc_flat = tf.concat(axis=1, values=[h_l_cc_flat, h_r_cc_flat])
        h_flat = (h_cc_flat)
        return h_flat
        ...
        h_mlo_flat = tf.concat(axis=1, values=[h_l_mlo_flat, h_r_mlo_flat])
        h_flat = (h_mlo_flat)
        return h_flat


def fc_layer(input_layer, network_type, number_of_units=128, activation_fn=tf.nn.relu, reuse=None, scope=None):
    """Fully connected layer"""
    if network_type == "CC":
        input_layer_cc = input_layer
        h_cc = tf.contrib.layers.fully_connected(inputs=input_layer_cc, num_outputs=number_of_units, activation_fn=activation_fn,
        ...
        input_layer_cc = input_layer
        h_mlo = tf.contrib.layers.fully_connected(inputs=input_layer_cc, num_outputs=number_of_units,
...
def softmax_layer(input_layer, network_type, number_of_outputs=3):
    """Softmax layer"""
    if network_type == "CC":
        input_layer_cc = input_layer
